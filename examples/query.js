...
var data = require("../jsonpath-test/data/store.json");

tap.test("first-level member", async function() {
  var results = jp.nodes(data, "$.store");
  tap.same(results, [{ path: ["$", "store"], value: data.store }]);
});
...
tap.test("member numeric literal matches string-numeric key", async function() {
  var data = {
    authors: { "1": "Herman Melville", "2": "J. R. R. Tolkien" }
  };
  var results = jp.nodes(data, "$.authors.1");
  tap.same(results, [{ path: ["$", "authors", 1], value: "Herman Melville" }]);
});
...
tap.test("root element gets us original obj", async function() {
  var results = jp.nodes(data, "$");
  tap.same(results, [{ path: ["$"], value: data }]);
});
...
tap.test("union on objects", async function() {
  tap.same(jp.query({ a: 1, b: 2, c: null }, '$..["a","b","c","d"]'), [
    1,
    2,
    null
  ]);
