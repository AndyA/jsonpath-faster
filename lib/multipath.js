...
class MultiPath extends Nest {
  ...
  addVisitor(path, visitor) {
    const { name, group } = this.makeAction(visitor);
    return this.addAction(
      path,
      ctx => `groups[${group}].push(() => ${name}(${ctx.lval()}, path))`
    );
  }

  addMutator(path, mutator) {
    const { name, group } = this.makeAction(mutator);
    return this.addAction(
      path,
      ctx =>
        `groups[${group}].push(() => ${ctx.lval()} = ${name}(${ctx.lval()}, path))`
    );
  }
  ...
  code() {
    return [
      ...
      jp.compiler.compileTokens(this.render(), { trackPath: true }),
