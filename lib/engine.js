...
const parseSlice = slice => {
  ...
  return norm(...parts).map((v, i) => {
    if (i === 0 && v === "") return 0; // allow default start
    if (i === 1 && v === "") return v; // allow absent end
...
const selectorCompiler = [
  ...
  },
  {
    ...
    gen: (ctx, tok) => {
      ...
      return {
        array: (lval, block) =>
          `for (let ${i} = 0; ${i} < ${lval}.length; ${i}++) ${undefer(
            block,
            i
          )};`,
        object: (lval, block) =>
  ...
  {
    when: { expression: { type: "slice" }, x: true, operation: "subscript" },
    gen: (ctx, tok) => {
      return {
        array: (lval, block) => {
          let [start, end, step] = parseSlice(tok.expression.value);
          const code = [];
          ...
          if (end === "") {
            ...
            if (end < 0) {
              code.push(
                `const ${e} = Math.min(${lval}.length${end}, ${lval}.length);`
              );
            } else {
              code.push(`const ${e} = Math.min(${end}, ${lval}.length);`);
            }
            end = e;
          }
