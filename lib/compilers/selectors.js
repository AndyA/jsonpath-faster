...
module.exports = [
  ...
  },
  {
    ...
    gen: (ctx, tok) => {
      return {
        // TODO do dynamic vivification here based on expression type
        code: (lval, block) => {
          const [i] = ctx.sym("i");
          const code = [
          ...
          if (tok.vv) {
            if (tok.vv === true) {
              code.push(
                `if (${lx} === undefined) { ${lx} = isNaN(${i}) ? {} : [] }; `
              );
            } else {
              code.push(`if (${lx} === undefined) { ${lx} = ${tok.vv} }; `);
            }
          } else {
            code.push(`if (${lx} !== undefined)`);
          }
          return [...code, `${block(i)};`].join("\n");
        }
      };
    }
  },
